...
/*
************************************************************************************************************************
*           Pedalboard plugin_map: parses the display list sent by mod-ui and draws it on the GLCD.
*
...
#define PLUGIN_MAP_RECORD_SEP      ';'

// record types, first character of each record
#define PLUGIN_MAP_REC_HEADER      'M'
#define PLUGIN_MAP_REC_NODE        'N'
#define PLUGIN_MAP_REC_PORT        'P'
#define PLUGIN_MAP_REC_EDGE        'E'
#define PLUGIN_MAP_REC_ADJACENCY   'A'

// node kinds, as sent on the wire
#define BM_PLUGIN          'p'
#define BM_HW_SOURCE       'i'
#define BM_HW_SINK         'o'

// port directions, as sent on the wire
#define BM_PORT_IN         'i'
#define BM_PORT_OUT        'o'

// signal types, kept as a bitmask so a layer can be toggled without refetching
#define BM_AUDIO           (1 << 0)
#define BM_MIDI            (1 << 1)
#define BM_CV              (1 << 2)
#define BM_ALL_LAYERS      (BM_AUDIO | BM_MIDI | BM_CV)

// navigation directions for plugin_map_step(). The panel has one encoder for the graph and
...
#define PLUGIN_MAP_PREV            0
#define PLUGIN_MAP_NEXT            1
 ...
 */
#define PLUGIN_MAP_VIEW_GRAPH      0
#define PLUGIN_MAP_VIEW_LIST       1
...
// (MOD_PLUGIN_MAP_WIN_PLUGINS, 12 by default) plus the hardware nodes their cables reach, so the cost is
// bounded by the window and not by how big the board is.
#define PLUGIN_MAP_MAX_NODES       28
#define PLUGIN_MAP_MAX_PORTS       72
#define PLUGIN_MAP_MAX_EDGES       48
#define PLUGIN_MAP_MAX_POINTS      8
#define PLUGIN_MAP_LABEL_SIZE      14
...
typedef struct PLUGIN_MAP_PORT_T {
    int16_t x, y;
    uint8_t node;           // index into plugin_map_t.nodes
    uint8_t pid;            // port id, unique within its node and direction
    uint8_t direction;      // BM_PORT_IN / BM_PORT_OUT
...
typedef struct PLUGIN_MAP_NODE_T {
    ...
    int16_t prev, next;
    char label[PLUGIN_MAP_LABEL_SIZE];
    char title[PLUGIN_MAP_TITLE_SIZE];   // for the title bar; falls back to label
} plugin_map_node_t;

typedef struct PLUGIN_MAP_EDGE_T {
    int16_t px[PLUGIN_MAP_MAX_POINTS];
    int16_t py[PLUGIN_MAP_MAX_POINTS];
    uint8_t n_points;
    uint8_t type;
...
typedef struct PLUGIN_MAP_T {
    ...
    plugin_map_node_t nodes[PLUGIN_MAP_MAX_NODES];
    plugin_map_port_t ports[PLUGIN_MAP_MAX_PORTS];
    plugin_map_edge_t edges[PLUGIN_MAP_MAX_EDGES];
    uint8_t n_nodes, n_ports, n_edges;

    // view
    glcd_rect_t view;       // clip rectangle on the panel
    int16_t offset_x, offset_y;     // scene -> panel pan
    int8_t selected;        // index into nodes, or BM_NONE
    uint8_t layers;         // which signal types to draw
    uint8_t view_mode;
    uint8_t blink_off;      // hold the selection highlight back, for a blink      // PLUGIN_MAP_VIEW_GRAPH or PLUGIN_MAP_VIEW_LIST

    // the nodes in alphabetical order, for the list view; rebuilt when the mode is set
    uint8_t order[PLUGIN_MAP_MAX_NODES];
     ...
     */
    uint8_t selectable[PLUGIN_MAP_MAX_NODES];
    uint8_t filtered;
} plugin_map_t;
...
// Array index of the node with this wire id, or BM_NONE.
/*
...
// The bypass state of one box, so the panel can redraw it from the host's answer instead
// of fetching the whole graph again for one changed flag.
...
// BM_NONE at the two ends of it. The id may name a box outside this window; resolve it
// with plugin_map_index_of() and refetch when that returns BM_NONE.
